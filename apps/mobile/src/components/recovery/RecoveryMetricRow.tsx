 ...
 * RecoveryMetricRow
 *
 * Recovery twin of the sleep `BiometricRow`: a full-row tile with the label +
 * value (+ status dot) on the left and a 7-day mini sparkline on the right.
 * Reads the selected day from RecoveryContext, pulls its own series from
 * `api.recovery.getRecoveryMetricSeries`, and pushes to /recovery-metric.
 */
...
export function RecoveryMetricRow({ metric, value }: Props) {
  const router = useRouter();
  const { selectedDay } = useRecoveryContext();
  const range = useMemo(() => rangeFor(selectedDay), [selectedDay]);

  const series = useQuery(api.recovery.getRecoveryMetricSeries, {
    metric: metric.id,
    from: range.from,
    to: range.to,
  });
  ...
  const content = (
    <View style={styles.card}>
      ...
      <View style={styles.iconBox}>
        <Ionicons name={metric.icon} size={15} color={dataColors.recovery.base} />
      </View>
  ...
  if (metric.allowDetail) {
    return (
      <Pressable
        onPress={() =>
          router.push({
            pathname: "/recovery-metric/[metric]",
...
const styles = StyleSheet.create({
  ...
  textCol: { width: 116, justifyContent: "center" },
  label: {
  ...
  chartWrap: { flex: 1, alignItems: "flex-end", justifyContent: "center" },
});
